...
void __merge2(T arr[], T aux[], int l, int mid, int r){
    ...
    // 由于aux的大小和arr一样, 所以我们也不需要处理aux索引的偏移量， 进一步节省了计算量
    for( int i = l ; i <= r; i ++ )
        aux[i] = arr[i];
...
template<typename T>
void __mergeSort3(T arr[], T aux[], int l, int r){
    ...
    int mid = (l+r)/2;
    __mergeSort3(arr, aux, l, mid);
    __mergeSort3(arr, aux, mid+1, r);
...
void mergeSort3(T arr[], int n){
    ...
    // 一次性申请aux空间，
    // 将这个辅助空间以参数的形式传递给完成归并排序的各个子函数
    T* aux = new T[n];

    __mergeSort3(arr, aux, 0, n-1);
